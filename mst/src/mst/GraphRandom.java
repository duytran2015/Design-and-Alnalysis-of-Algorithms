...
class GraphRandom extends Mst {
    ...
    public GraphRandom(int n, int density) {
        ...
        // calculating density
        den = (int) Math.ceil(temp * density / 100);
    ...
    public void dfs(int v, int[] dfscheck) {
        for (Neighbor nbr = vertexList[v].adjList; nbr != null; 
            nbr = nbr.next) 
        {
    ...
    public void checkConnected(int n, int den) {
        ...
        dfs(0, dfscheck);
        for (int v = 0; v < vertexList.length; v++) {
            // recursively calling the check connected function to fix
            // unconnected graph
            if (dfscheck[v] == 0){ 
                //checkConnected(n, den);
                connected = false;
    ...
    public void addNeighbors(int n, int den) {
        for (int v = 0; v < vertexList.length; v++) {
        ...
        for (int i = 0; i < den; i++) {
            ...
            if (v1 != v2) {
                Neighbor nbr;
                // checking if the selected vertices 
                // are not already neighbors
                for (nbr = vertexList[v2].adjList; nbr != null; 
                    nbr = nbr.next) 
                {
